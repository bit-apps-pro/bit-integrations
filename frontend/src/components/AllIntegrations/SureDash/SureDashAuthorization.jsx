...
export default function SureDashAuthorization({
  ...
  const handleInput = e => {
    const newConf = { ...sureDashConf }
    newConf[e.target.name] = e.target.value
    setSureDashConf(newConf)
  }
    ...
    <div
      ...
      {isLoading === 'auth' && (
        <div className="flx mt-5">
          ...
          {__('Checking if SureDash is authorized!!!', 'bit-integrations')}
        </div>
      )}
      ...
      {showAuthMsg && isAuthorized && !isLoading && (
        <div className="flx mt-5" style={{ width: 900 }}>
          ...
          <div className="mt-2">{__('SureDash is activated', 'bit-integrations')}</div>
        </div>
      )}
