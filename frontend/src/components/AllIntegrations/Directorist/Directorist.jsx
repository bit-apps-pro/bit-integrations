...
export default function Directorist({ formFields, setFlow, flow, allIntegURL }) {
  ...
  const [directoristConf, setDirectoristConf] = useState({
    name: 'Directorist',
    type: 'Directorist',
    field_map: [{ formField: '', directoristField: '' }],
    actions: {},
    mainAction: ''
  })
  ...
  const nextPage = val => {
    ...
    if (val === 3) {
      ...
      if (directoristConf.name !== '' && directoristConf.field_map.length > 0) {
        setStep(val)
      }
    } else {
      setStep(val)
  ...
  return (
    <div>
      ...
      <div
        ...
        <button
          onClick={() => nextPage(3)}
          disabled={directoristConf.field_map.length < 1}
      ...
      <IntegrationStepThree
        step={step}
        saveConfig={() =>
          saveIntegConfig(flow, setFlow, allIntegURL, directoristConf, navigate, '', '', setIsLoading)
        }
        isLoading={isLoading}
