...
export default function WebbaBookingAuthorization({
  ...
  const handleInput = e => {
    const newConf = { ...webbaBookingConf }
    newConf[e.target.name] = e.target.value
    setWebbaBookingConf(newConf)
  }
  ...
  return (
    <div
      ...
      <TutorialLink title="Webba Booking" links={tutorialLinks?.webbaBooking || {}} />
      ...
      {isLoading === 'auth' && (
        <div className="flx mt-5">
          ...
          {__('Checking if Webba Booking is authorized!!!', 'bit-integrations')}
        </div>
      )}
      ...
      {showAuthMsg && !isAuthorized && !isLoading && (
        <div className="flx mt-5" style={{ width: 900, justifyContent: 'center' }}>
          <div className="txt-center">
            ...
            <div className="mt-2">
              {__('Webba Booking is not activated or not installed', 'bit-integrations')}
            </div>
          </div>
        </div>
      )}
      ...
      {showAuthMsg && isAuthorized && !isLoading && (
        <div className="flx mt-5" style={{ width: 900 }}>
          ...
          <div className="mt-2">{__('Webba Booking is activated', 'bit-integrations')}</div>
        </div>
      )}
      ...
      <button
        ...
        {isAuthorized
          ...
          : __('Connect to Webba Booking', 'bit-integrations')}
