...
export default function B2BKingAuthorization({
  ...
  const authorizeHandler = () => {
    setIsLoading('auth')
    bitsFetch({}, 'b2bking_authorize').then(result => {
      if (result?.success) {
        setIsAuthorized(true)
        setSnackbar({ show: true, msg: __('Connected with B2BKing Successfully', 'bit-integrations') })
      }
      setIsLoading(false)
      setShowAuthMsg(true)
    })
  ...
  const handleInput = e => {
    const newConf = { ...b2bKingConf }
    newConf[e.target.name] = e.target.value
    setB2BKingConf(newConf)
  }
