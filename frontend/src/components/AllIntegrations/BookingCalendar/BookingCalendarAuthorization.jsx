...
export default function BookingCalendarAuthorization({
  ...
  const handleInput = e => {
    if (!setBookingCalendarConf) return
    setBookingCalendarConf({ ...bookingCalendarConf, [e.target.name]: e.target.value })
  }
  ...
  return (
    <div
      ...
      <TutorialLink linkKey="bookingCalendar" />
      ...
      {isLoading === 'auth' && (
        <div className="flx mt-5">
          ...
          {__('Checking if Booking Calendar is authorized!!!', 'bit-integrations')}
        </div>
      )}
      ...
      {showAuthMsg && !isAuthorized && !isLoading && (
        <div className="flx mt-5" style={{ width: 900, justifyContent: 'center' }}>
          <div className="txt-center">
            ...
            <div className="mt-2">
              {__('Booking Calendar is not activated or not installed', 'bit-integrations')}
            </div>
          </div>
        </div>
      )}
      ...
      {showAuthMsg && isAuthorized && !isLoading && (
        <div className="flx mt-5" style={{ width: 900 }}>
          ...
          <div className="mt-2">{__('Booking Calendar is activated', 'bit-integrations')}</div>
        </div>
      )}
        ...
        <>
          <button
            ...
            {isAuthorized
              ...
              : __('Connect to Booking Calendar', 'bit-integrations')}
