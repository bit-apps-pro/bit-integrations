...
import { $actionConf, $appConfigState, $flowStep, $newFlow } from '../../../GlobalStates'
...
import {
  ...
  GiveWpStateIH,
  LifterLmsStateIH,
  MasterStudyLmsStateIH,
  PaidMembershipProStateIH,
...
export const saveIntegConfig = async (
  ...
  const btcbi = getRecoil($appConfigState)
  ...
  if (['Mail', 'Telegram', 'WhatsApp', 'Trello', 'Google Calendar'].includes(confTmp?.type)) {
...
export const saveActionConf = async ({
  ...
  if (['Mail', 'Telegram', 'WhatsApp', 'Trello', 'Google Calendar'].includes(conf?.type)) {
    action = edit ? 'flow/sanitize_post_content/update' : 'flow/sanitize_post_content/save'
  }
