...
export default function GoogleDriveAuthorization({
  ...
  const note = `
    <h4>${__('Google Drive OAuth setup', 'bit-integrations')}</h4>
    <ul>
      ...
      <li>${__('Enable Google Drive API and authorize with required scope.', 'bit-integrations')}</li>
    </ul>
  `

  return (
    <Authorization
      config={googleDriveConf}
      setConfig={setGoogleDriveConf}
      step={step}
      setStep={setStep}
      isInfo={isInfo}
      tutorialTitle="Google Drive"
      tutorialLinks={tutorialLinks?.googleDrive || {}}
      authDetails={{
