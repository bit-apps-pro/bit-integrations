...
export default function ZohoCRMAuthorization({
  ...
  const loadModules = useCallback(
    async connectionId => {
      const nextConf = connectionId ? { ...crmConf, connection_id: connectionId } : crmConf
      if (!nextConf.module) {
        refreshModules(formID, nextConf, setCrmConf, setIsLoading, setSnackbar)
      }
    },
    [crmConf, formID, setCrmConf, setIsLoading, setSnackbar]
  )

  const handleSetStep = useCallback(
    value => {
      if (value === 2 && !crmConf.module) {
        loadModules()
      }
      setstep(value)
    },
    [crmConf.module, loadModules, setstep]
  )
  ...
  return (
    <Authorization
      ...
      isInfo={isInfo}
      tutorialTitle="Zoho CRM"
      tutorialLinks={tutorialLinks?.zohoCRM || {}}
      authDetails={{
