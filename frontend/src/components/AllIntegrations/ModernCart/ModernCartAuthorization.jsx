...
export default function ModernCartAuthorization({
  ...
  const handleInput = e => {
    const newConf = { ...modernCartConf }
    newConf[e.target.name] = e.target.value
    setModernCartConf(newConf)
  }
  ...
  return (
    <div
      ...
      <TutorialLink linkKey="modernCart" />
      ...
      {isLoading === 'auth' && (
        <div className="flx mt-5">
          ...
          {__('Checking if Modern Cart is authorized!!!', 'bit-integrations')}
        </div>
      )}
      ...
      {showAuthMsg && !isAuthorized && !isLoading && (
        <div className="flx mt-5" style={{ width: 900, justifyContent: 'center' }}>
          <div className="txt-center">
            ...
            <div className="mt-2">
              {__('Modern Cart is not activated or not installed', 'bit-integrations')}
            </div>
          </div>
        </div>
      )}
      ...
      {showAuthMsg && isAuthorized && !isLoading && (
        <div className="flx mt-5" style={{ width: 900 }}>
          ...
          <div className="mt-2">{__('Modern Cart is activated', 'bit-integrations')}</div>
        </div>
      )}
      ...
      <button
        ...
        {isAuthorized
          ...
          : __('Connect to Modern Cart', 'bit-integrations')}
